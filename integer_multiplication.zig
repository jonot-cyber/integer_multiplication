...
fn calcTerm(n: u16) void {
    ...
    for (1..n) |i| {
        ...
        var cost = Cost{
            .n = n,
            .value = if (larger == smaller) costs[larger].lowest.value + 1 else costs[larger].lowest.value + costs[smaller].lowest.value + 1,
...
pub fn main() !void {
    ...
    for (2..256) |i| {
        ...
        try out.print("{d}: {d}\n", .{ i, costs[i].lowest.value });
    }
    try out.print("{d}\n", .{sum});
    try printPath(&costs[7].lowest);
}
